...
export function Progress({ value, total, color = Colors.accent, height = 7, showLabel = false }: Props) {
...
export function RingProgress({ value, total, size = 44, color = Colors.accent }: RingProps) {
  ...
  return (
    <View style={{ width: size, height: size }}>
      <Text style={[styles.ringPct, { fontSize: size * 0.22, color }]}>{Math.round(pct * 100)}%</Text>
